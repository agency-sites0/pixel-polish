...
export const Route = createFileRoute("/insights/$slug")({
  ...
  notFoundComponent: () => (
    <section className="py-32 text-center">
      ...
      <Link to="/insights" className="mt-6 inline-flex text-sm font-medium">
        Back to journal
      </Link>
    </section>
  ),
        ...
        <div className="mx-auto max-w-2xl space-y-8 px-5">
          {article.content.map((p, i) => (
            <div key={i}>
              ...
              <p
                className={`text-base leading-relaxed text-foreground/85 ${p.heading ? "mt-4" : ""}`}
              >
                {p.body}
              </p>
            </div>
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {related.map((a) => (
              <Link
                ...
                to="/insights/$slug"
                params={{ slug: a.slug }}
                className="group block overflow-hidden rounded-3xl border border-border bg-card transition-all hover:-translate-y-1"
              >
